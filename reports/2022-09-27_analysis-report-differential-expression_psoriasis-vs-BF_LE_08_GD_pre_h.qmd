...
subtitle: "BF_LE_08_GD_pre_h data vs Psoriasis data"
...
# Idents(obj_08) <- "predicted.id.Cell_type_nicknames"
...
# obj_merged_08 <- RunPCA(obj_merged_08, npcs = 30)
# obj_merged_08 <- RunUMAP(obj_merged_08, dims = 1:30)
# obj_merged_08 <- RunUMAP(obj_merged_08, dims = 1:30)
# s.genes <- cc.genes$s.genes
# g2m.genes <- cc.genes$g2m.genes
# obj_merged_08 <- CellCycleScoring(obj_merged_08, s.features = s.genes, g2m.features = g2m.genes, set.ident = TRUE)
filename <- here(output_data_path, "merged-seurat-obj_obj-08.RData")
# save(obj_merged_08, file = filename)
load(filename)
...
obj_merged_08$temp.ident <- "psoriasis"
obj_merged_08$temp.ident[grep("BF_LE_08_GD", colnames(obj_merged_08))] <- "lupus"
DefaultAssay(obj_merged_08) <- "RNA"
...
p1 <- VlnPlot(obj_merged_08, features = "nFeature_RNA", group.by = "temp.ident") +
  stat_compare_means(method = "wilcox.test")
p2 <- VlnPlot(obj_merged_08, features = "nCount_RNA", group.by = "temp.ident") +
  stat_compare_means(method = "wilcox.test")
p3 <- VlnPlot(obj_merged_08, features = "percent.mt", group.by = "temp.ident") +
  stat_compare_means(method = "wilcox.test")
pw <- p1 | p2 | p3 
...
DimPlot(obj_merged_08, group.by = "predicted.id.Cell_type_nicknames", label = TRUE) + ggtitle("UMAP of merged psoriatic and 08 data set, colored by cell types")
```

```{r, fig.width=15, fig.height=5}
p1 <- DimPlot(obj_merged_08, group.by = "temp.ident") + ggtitle("experiment")
p2 <- DimPlot(obj_merged_08, group.by = "Phase") + ggtitle("cell cycle phase")
p3 <- FeaturePlot(obj_merged_08, "nFeature_RNA") + ggtitle("expressed genes per cell")
...
top5_marker_inflammatory_plot_up <- top5_marker_inflammatory_plot_up[names(top5_marker_inflammatory_plot_up) %in% inflammatory]
marker_idents_up <- names(top5_marker_inflammatory_plot_up)
top5_marker_inflammatory_plot_up_unlist <- top5_marker_inflammatory_plot_up %>%
  map(~pull(., feature)) %>%
  unlist()

top5_marker_inflammatory_plot_down  <- map(de_integr_08,~top_n(., n = -5, wt = avg_log2FC))
top5_marker_inflammatory_plot_down <-  top5_marker_inflammatory_plot_down[names(top5_marker_inflammatory_plot_down) %in% inflammatory]
marker_idents_down <- names(top5_marker_inflammatory_plot_down)
top5_marker_inflammatory_plot_down_unlist <- top5_marker_inflammatory_plot_down %>%
  map(~pull(., feature)) %>%
  unlist()
...
DotPlot(obj_merged_08_inflammatory, features = unique(top5_marker_inflammatory_plot_up_unlist), idents = marker_idents_up, split.by = "temp.ident", dot.scale = 6,  group.by = "predicted.id.Cell_type_nicknames", cols = c("red", "blue")) +
  theme(
    axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 10)
) +
  ggtitle("up in psoriasis")

DotPlot(obj_merged_08_inflammatory, features = unique(top5_marker_inflammatory_plot_down_unlist), idents = marker_idents_up, group.by = "predicted.id.Cell_type_nicknames", split.by = "temp.ident", dot.scale = 6, cols = c("red", "blue")) +
  theme(
    axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 10)
)+
  ggtitle("down in psoriasis")
...
top5_marker_inflammatory_plot_up  <- map(de_integr_08,~top_n(., n = 6, wt = avg_log2FC)) %>%
  map(~pull(., feature))

for(group in intersect(unique(obj_merged_08_inflammatory$predicted.id.Cell_type_nicknames), names(top5_marker_inflammatory_plot_up))){
  print(VlnPlot(
    object = obj_merged_08_inflammatory, 
    ...
    idents = group
      ...
      plot_annotation(title = paste(group, "psoriasis up")) )
}
...
```{r, fig.height=3, fig.width=15}
top5_marker_inflammatory_plot_down  <- map(de_integr_08,~top_n(., n = -6, wt = avg_log2FC)) %>%
  map(~pull(., feature)) 

for(group in intersect(unique(obj_merged_08_inflammatory$predicted.id.Cell_type_nicknames), names(top5_marker_inflammatory_plot_down))){
  print(VlnPlot(
    object = obj_merged_08_inflammatory, 
    ...
    idents = group
      ...
      plot_annotation(title = paste(group, "lupus up")) + plot_layout(guides = 'collect'))
}
...
cytokines <- intersect(interleukins, rownames( obj_merged_08_inflammatory))
cytokines <- unique(c(cytokines,"IFNG", "TNF", "LTA", "TGFA", "TGFB1", "TGFB2", "TGFB3" )) 
cytokines <- sort(cytokines)

DotPlot(
    obj_merged_08_inflammatory, 
    features = cytokines, 
    group.by = "predicted.id.Cell_type_nicknames",
    split.by = "temp.ident",
    dot.scale = 6, 
...
DotPlot(obj_merged_08_inflammatory, features = genes_of_interest, group.by = "predicted.id.Cell_type_nicknames", split.by = "temp.ident", dot.scale = 6, cols = c("red", "blue")) +
  theme(
    axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 10)
) + 
  coord_flip()
...
```{r, fig.height=3, fig.width=7, results = 'asis'}

for(gene in intersect(genes_of_interest, rownames(obj_merged_08_inflammatory))){
  
    tb <- obj_merged_08_inflammatory@meta.data %>%
        data.frame(
          t(as.matrix(obj_merged_08_inflammatory@assays$RNA@data[intersect(genes_of_interest, rownames(obj_merged_08_inflammatory@assays$RNA@data)),]))
        ) %>%
  ...
  #   filter(str_detect(group1, regex("obj08"))) %>% 
  #   mutate(group1 = gsub("_obj08", "", group1 )) %>% 
  ...
  stat_table <-  tb %>% 
    ...
    mutate(data_set = str_replace_all(data_set, 
            pattern = "obj08", replacement = "lupus")) %>% 
  ...
  ft <- set_caption(ft, caption =  paste("Summary statistics of ",gene," expression per celltype and dataset."))
   ...
   # p_adj_sort <- stat_table[na.omit(match(levels(obj_merged_08_inflammatory$predicted.id.Cell_type_nicknames), stat_table$group1)),]$signif 
  ...
  stat_compare_means(label = "p.signif",comparisons = c("objintegr", "obj08"))  
  # annotate(
  #   "text", 
  #   x = seq_along(p_adj_sort), 
  #   y = (max(GetAssayData(obj_merged_08_inflammatory, assay = "RNA", slot = "data")[gene,])) -0.01,
  #   label = p_adj_sort,
  #   color = "red",
  #   size = 10
  #   )
  ...
  #   drop_na() %>% 
  #   print()
  # stat_table[na.omit(match(levels(obj_merged_08_inflammatory$predicted.id.Cell_type_nicknames), stat_table$group1)),]$p.adj
...
for(gene in intersect(genes_of_interest, rownames(obj_merged_08_inflammatory))){
  ...
  data_use <- GetAssayData(obj_merged_08_inflammatory, assay = "RNA", slot = "data")[gene,]
  table_use <- table(obj_merged_08_inflammatory$temp.ident, data_use>0)
...
Idents(obj_merged_08_inflammatory) <- "temp.ident"
obj_merged_08_inflammatory_lupus<- subset(obj_merged_08_inflammatory, idents = "lupus")

for(gene in intersect(genes_of_interest, rownames(obj_merged_08_inflammatory_lupus))){
  
  data_use <- GetAssayData(obj_merged_08_inflammatory_lupus, assay = "RNA", slot = "data")[gene,]
  table_use <- table(obj_merged_08_inflammatory_lupus$predicted.id.Cell_type_nicknames, data_use>0)
...
Idents(obj_merged_08_inflammatory) <- "temp.ident"
obj_merged_08_inflammatory_psoriasis <- subset(obj_merged_08_inflammatory, idents = "psoriasis")

for(gene in intersect(genes_of_interest, rownames(obj_merged_08_inflammatory_psoriasis))){
  
  data_use <- GetAssayData(obj_merged_08_inflammatory_psoriasis, assay = "RNA", slot = "data")[gene,]
  table_use <- table(obj_merged_08_inflammatory_psoriasis$predicted.id.Cell_type_nicknames, data_use>0)
